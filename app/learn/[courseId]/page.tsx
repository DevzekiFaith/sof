...
export default function CoursePlayerPage() {
  ...
  const completedStages = modules.reduce((acc, _, mi) => {
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          {currentModuleData.activities?.length > 0 ? (
            <div className="space-y-4">
              {currentModuleData.activities.map((activity, i) => {
                const isChecked = !!checkedActivities[i];
                return (
                  <div 
                    key={i}
                    onClick={() => toggleActivity(i)}
                    className={`flex items-start gap-4 p-5 rounded-xl border transition-all cursor-pointer select-none ${
                      isChecked 
                        ? 'bg-[#1ed760]/10 border-[#1ed760]' 
                        : 'bg-[#181818] border-[#282828] hover:bg-[#282828]'
                    }`}
                  >
                    <div className={`w-6 h-6 mt-0.5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                      isChecked ? 'bg-[#1ed760] border-[#1ed760] text-black' : 'border-[#b3b3b3]'
                    }`}>
            ...
            {modules.map((mod, mi) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left flex items-start gap-3 p-3 rounded-md transition-all duration-200 ${
                    isActive
                ...
                >
                  <span className={`w-7 h-7 flex-shrink-0 rounded flex items-center justify-center text-xs font-bold mt-0.5 ${
                    status === "complete" ? "bg-[#1ed760]/20 text-[#1ed760]" :
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isActive && (
                      <div className="flex gap-1 mt-1.5 flex-wrap">
                        {STAGES.map((s) => (
                          <span key={s.key} className={`text-xs px-1.5 py-0.5 rounded font-medium ${
                            isStageComplete(courseId, mi, s.key)
              ...
              {STAGES.map((stage, si) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-3 rounded-t-md text-sm font-bold transition-all duration-200 whitespace-nowrap flex-shrink-0 border-b-2 ${
                      active
                ...
                <div className="flex gap-3">
                  {!isCurrentStageComplete && currentUser && (
                    <button
                      ...
                      className={`px-8 py-3.5 font-bold rounded-full transition-all duration-200 uppercase tracking-wide ${
                        justCompleted
              ...
              {modules.map((mod, mi) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left flex items-center gap-3 p-4 rounded-md transition-all ${
                      isActive ? "bg-[#282828] border-transparent" :
                      status === "locked" ? "opacity-40 cursor-not-allowed border-transparent text-[#a7a7a7]" :
                      "border-transparent hover:bg-[#1a1a1a] text-[#b3b3b3]"
                    }`}
                  >
                    <span className={`w-8 h-8 flex-shrink-0 rounded flex items-center justify-center text-sm font-bold ${
                      status === "complete" ? "bg-[#1ed760]/20 text-[#1ed760]" :
                      status === "in-progress" ? "bg-blue-500/20 text-blue-400" :
                      "bg-white/5 text-gray-500"
                    }`}>
