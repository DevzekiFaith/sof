...
import { BookOpen, Globe, Zap, Wrench, RotateCcw, Target } from "lucide-react";
            ...
            <AnimatedSection delay={100}>
              <div>
                ...
                <ul className="space-y-4">
                  {[
                    { label: "Learn", icon: <BookOpen className="w-5 h-5 text-[#1ed760]" />, desc: "Deep dive into core formation content" },
                    { label: "Practice", icon: <Zap className="w-5 h-5 text-[#1ed760]" />, desc: "Active exercises to build muscle memory" },
                    { label: "Reflect", icon: <RotateCcw className="w-5 h-5 text-[#1ed760]" />, desc: "Internalize lessons through guided reflection" },
                    { label: "Apply", icon: <Target className="w-5 h-5 text-[#1ed760]" />, desc: "Real-world application in your daily life" },
                  ].map((stage, i) => (
                    <li key={i} className="flex items-center gap-4 group bg-[#181818] p-4 rounded-md hover:bg-[#282828] transition-colors">
                      <div className="w-10 h-10 rounded-full bg-[#1ed760]/10 flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-110">
                        {stage.icon}
                      </div>
                      <p className="text-[#b3b3b3]"><strong className="text-white">{stage.label}</strong> - {stage.desc}</p>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#181818] p-6 rounded-md border border-[#282828]">
              ...
              <div className="mb-6">
                ...
                <div className="text-3xl font-black text-[#1ed760]">1.2M+</div>
              </div>
              <div>
