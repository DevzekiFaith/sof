...
import { Users, MessageSquare, Plus, Download, CheckCircle2, FileText, Send, Sparkles, ShieldCheck, ExternalLink, ArrowRight, Zap } from 'lucide-react';
...
export default function CommunityPage() {
  ...
  const [activeTab, setActiveTab] = useState<'vip' | 'groups' | 'forum'>('vip');
  ...
  return (
    <div className="min-h-screen bg-[#080b12] text-white p-4 md:p-8 font-sans selection:bg-[#60a5fa]/30 selection:text-[#60a5fa]">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        {/* Pinterest Style Header Navigation & Tabs */}
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
          ...
          <div className="flex gap-2 p-1.5 bg-[#0f1422] rounded-full border border-white/5">
            <button
              onClick={() => setActiveTab('vip')}
              className={`flex items-center gap-2 px-5 py-2 rounded-full font-bold text-xs transition-all duration-300 ${
                activeTab === 'vip' 
                  ? 'bg-gradient-to-r from-[#60a5fa] to-blue-600 text-black shadow-lg shadow-[#60a5fa]/25' 
                  : 'text-zinc-400 hover:text-white'
              }`}
            >
              <Zap size={14} />
              VIP Inner Circle & Manuscripts
            </button>
            <button
        ...
        {/* TAB 1: VIP INNER CIRCLE & MANUSCRIPTS UNLOCK */}
        {activeTab === 'vip' && (
          <div className="space-y-14">
            
            {/* Pinterest / Dribbble Hero Spotlight Banner */}
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#0d121f] via-[#12192b] to-[#0a0e18] border border-white/10 p-8 md:p-14 shadow-2xl">
              ...
              <div className="relative z-10 grid lg:grid-cols-12 gap-8 items-center">
                ...
                {/* Featured 3D Book Preview Stack (Right side visual) */}
                <div className="lg:col-span-5 relative flex justify-center items-center">
                  <div className="relative w-48 h-64 md:w-56 md:h-76 group cursor-pointer" onClick={() => setSelectedPdfId(selectedPdf.id)}>
                    ...
                    <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl border border-white/20 group-hover:scale-105 transition-transform duration-500">
                      <Image 
            ...
            {!isSubmitted ? (
              <div className="grid lg:grid-cols-12 gap-10 items-start">
                ...
                {/* Pinterest Modern Glass Form (5 Cols) */}
        ...
        {activeTab === 'groups' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-bold">Community Study Groups</h2>
              <button
                onClick={() => setShowCreateGroup(true)}
                className="flex items-center gap-2 bg-[#60a5fa] text-black px-5 py-2.5 rounded-full font-bold text-sm hover:bg-[#60a5fa]/90 transition-colors"
              >
            ...
            <div className="space-y-4">
              {loading ? (
                <div className="text-center py-8 text-zinc-500">Loading study groups...</div>
              ) : studyGroups.length === 0 ? (
                <div className="text-center py-12 bg-[#0f1422] rounded-2xl border border-white/5 text-zinc-400">
                  No public study groups created yet. Be the first to launch one!
                </div>
              ) : (
                studyGroups.map((group) => (
                  <div key={group.id} className="bg-[#0f1422] p-6 rounded-2xl border border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                      ...
                      {group.isMember ? (
                        <button
                          ...
                          className="px-4 py-2 bg-red-500/10 text-red-400 border border-red-500/20 text-xs font-bold rounded-full hover:bg-red-500/20 transition-colors"
                        >
                          Leave Group
                        </button>
                      ) : (
                        <button
                          ...
                          className="px-4 py-2 bg-[#60a5fa] text-black text-xs font-bold rounded-full hover:bg-[#60a5fa]/90 transition-colors"
                        >
                          Join Group
                        </button>
                      )}
        ...
        {activeTab === 'forum' && (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {loading ? (
                <div className="text-center py-8 text-zinc-500">Loading forum discussions...</div>
              ) : forumPosts.length === 0 ? (
                <div className="text-center py-12 bg-[#0f1422] rounded-2xl border border-white/5 text-zinc-400">
                  No community forum posts yet. Start a discussion!
                </div>
              ) : (
                forumPosts.map((post) => (
                  <div key={post.id} className="bg-[#0f1422] p-6 rounded-2xl border border-white/5 space-y-3">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${
                          post.isLiked ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'
                        }`}
                      >
                        ❤️ {post.likes}
                      </button>
        ...
        {showCreateGroup && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-[#111726] border border-white/10 p-6 rounded-3xl max-w-md w-full shadow-2xl">
              <h2 className="text-2xl font-bold mb-4 text-white">Create Study Group</h2>
              <form onSubmit={(e) => {
                e.preventDefault();
                ...
                <div className="space-y-4">
                  <input
                    ...
                    required
                    className="w-full bg-[#080b12] border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-[#60a5fa]"
                  />
                  <textarea
                    name="description"
                ...
                <div className="flex gap-2 mt-6">
                  <button
                    ...
                    className="flex-1 px-4 py-3 bg-[#1e2738] text-white font-bold rounded-xl hover:bg-[#28344b] transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 px-4 py-3 bg-[#60a5fa] text-black font-bold rounded-xl hover:bg-[#60a5fa]/90 transition-colors"
                  >
                    Create
                  </button>
                </div>
        ...
        {showCreatePost && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-[#111726] border border-white/10 p-6 rounded-3xl max-w-md w-full shadow-2xl">
              <h2 className="text-2xl font-bold mb-4 text-white">Create Forum Post</h2>
              <form onSubmit={(e) => {
                e.preventDefault();
                const formData = new FormData(e.currentTarget);
                createForumPost(
