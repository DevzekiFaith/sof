        ...
        <div className="container mx-auto px-4 py-16">
          <div className="max-w-2xl mx-auto text-center">
            <div className="w-24 h-24 bg-[#282828] rounded-full flex items-center justify-center mx-auto mb-6">
              <ShoppingBag size={48} className="text-[#b3b3b3]" />
            </div>
            ...
            <Link
              href="/courses"
              className="inline-flex items-center gap-2 px-6 py-3 bg-[#1ed760] text-black font-bold rounded-full hover:scale-105 transition-transform"
            >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-[#b3b3b3]">₦{(item.priceUSD || 0) * 1500}</span>
                    </div>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="text-[#b3b3b3] hover:text-red-500 transition-colors self-start"
                  >
                    <Trash2 size={20} />
                  </button>
                </div>
              ...
              <div className="bg-[#181818] rounded-xl border border-[#282828] p-6 sticky top-4">
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex justify-between text-sm">
                    <span className="text-[#b3b3b3]">Subtotal ({cartCount} items)</span>
                    <span className="font-bold">${cartTotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-[#b3b3b3]">NGN Equivalent</span>
                    <span className="font-bold">₦{cartTotalNGN.toLocaleString()}</span>
                  </div>
                  <hr className="border-[#282828]" />
                  <div className="flex justify-between text-lg">
                    <span className="font-bold">Total</span>
                    <span className="font-black text-[#1ed760]">${cartTotal.toFixed(2)}</span>
                  </div>
                </div>
                <button
                  onClick={handleCheckout}
                  className="w-full py-4 bg-[#1ed760] text-black font-bold rounded-full hover:scale-105 transition-transform flex items-center justify-center gap-2"
                >
                  Proceed to Checkout <ArrowRight size={16} />
                </button>
                <Link
                  href="/courses"
                  className="block w-full mt-3 text-center text-sm text-[#b3b3b3] hover:text-white transition-colors"
                >
                  Continue Shopping
                </Link>
              </div>
