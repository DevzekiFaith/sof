    ...
    <>
      ...
      >
        ...
        <div className={`bg-gradient-to-br ${course.bgGradient} p-8 pb-10 relative overflow-hidden`}>
          ...
          <div className="absolute -top-10 -right-10 w-48 h-48 bg-[#60a5fa]/20 rounded-full blur-3xl pointer-events-none" />
          ...
          {course.featured && (
            <div className="mb-3 relative z-10">
              <span className="px-3 py-1 bg-[#60a5fa] text-black text-xs font-black rounded-full uppercase tracking-wider">
                Popular
              </span>
            </div>
          )}
              ...
              <ul className="space-y-3">
                {outcomes.map((outcome, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <span className="mt-0.5 w-5 h-5 flex-shrink-0 bg-[#60a5fa]/10 rounded-full flex items-center justify-center border border-[#60a5fa]/20">
                ...
                <div key={i} className="bg-[#181818] rounded-xl p-4 border border-[#282828]">
                  <div className="flex items-center gap-1 mb-2">
                    {[...Array(review.rating)].map((_, j) => (
                      <Star key={j} className="w-4 h-4 fill-[#60a5fa] text-[#60a5fa]" />
                    ))}
                  </div>
                  <p className="text-sm text-[#e5e5e5] mb-3 leading-relaxed">{review.text}</p>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-[#282828] flex items-center justify-center text-xs font-bold text-[#60a5fa]">
        ...
        <div className="sticky bottom-0 bg-[#121212]/95 backdrop-blur-md border-t border-[#282828] p-6 shadow-2xl flex flex-col gap-3">
          {canAccessCourse ? (
            <Link href={`/learn/${course.id}`} className="block w-full" onClick={onClose}>
              <button className="w-full py-4 bg-[#60a5fa] hover:scale-105 text-black font-bold text-lg rounded-full shadow-lg shadow-black/40 transition-all flex items-center justify-center gap-2 group">
                Start Course
              </button>
            </Link>
          ) : (
